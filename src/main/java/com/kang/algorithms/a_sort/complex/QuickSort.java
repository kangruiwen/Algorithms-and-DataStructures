 ...
 * 
 * 这个快速排序是最初级的快排算法，虽然对打乱了的数据排序效率高，但是有以下几个问题
 * 1.在面对一个已经几乎排好序的数组，算法复杂度会退化为O(n^2)级别的一个算法，这是因为，每次我们都选取数组的第一个元素为基准，在后边与基准对比分治的时候，往往由于数据近乎有序而造成分开的两个
 * 部分严重不均衡，理想中平衡的二叉树是最好的，然后这个二叉树近乎退化为一个链表，每次又对这个链表再次遍历，所以算法复杂度会退化：可以看QucikSortOptimize
 * 
 */
